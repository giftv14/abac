...
void UserConfigs::Create(std::string data)
{
	...
	const auto req = new Request("http://193.26.14.16/cloud_configs/");
	req->Perform(szQuery, strlen(szQuery));
	delete req;
...
void UserConfigs::Delete()
{
	...
	sprintf(szQuery, "action=delete&creator_id=%s&config_id=%s", std::to_string(m_UID).data(), std::to_string(m_CID).data());
...
void UserConfigs::Reload(std::vector<std::string>& out)
{
	...
	const auto req = new Request("http://193.26.14.16/cloud_configs/");
	std::string reqRes = req->Perform(szQuery, strlen(szQuery));
	delete req;

	if (reqRes.size() > 0)
	{
		const nlohmann::json j = nlohmann::json::parse(reqRes);
