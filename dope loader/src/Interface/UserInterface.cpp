...
void UserInterface::Display()
{
	...
	std::vector<std::string> playerEspItems{ "2D Filled", "3D", "3D Filled" };
				...
				if (animStage == 4) {
					std::thread([&] {
						...
						if (!bLoginProceceed) {
							...
							auto auth = new Authentification(Authentification::GetHwid(), Instance<Globals>::Get()->User.m_Token);
							auth->Login(bLogged);
							delete auth;
						}
...
#else
						globals->User.userName = "Guest User";
						bLoginProceceed = true;
						bLogged = true;
						iLoadingPercentage = 40;
#endif
...
#ifndef _DEBUG
							std::string fullBuf = Instance<Globals>::Get()->User.dllOne + Instance<Globals>::Get()->User.dllTwo;
							Instance<InjectionHelper>::Get()->Inject(Instance<Globals>::Get()->m_Pid, fullBuf.data(), Instance<Globals>::Get()->User.dllSize);
							Instance<Globals>::Get()->User.dllOne.clear();
							Instance<Globals>::Get()->User.dllTwo.clear();
							fullBuf.clear();
#else
